...
class AudioClassificationDataset(Dataset):
    ...
    def __init__(
        ...
        if split_long:
            ...
            self.df[start_time_col] = pd.to_numeric(
                self.df[start_time_col], errors="coerce"
            )
            self.df[end_time_col] = pd.to_numeric(
                self.df[end_time_col], errors="coerce"
            )
    ...
    def _split_long_annotations(
        ...
        for _, row in df.iterrows():
            ...
            if duration <= self.audio_duration:
                ...
                for i in range(n_clips):
                    ...
                    new_end = min(
                        start_float + (i + 1) * self.audio_duration, end_float
                    )
        ...
        result_df[self.start_time_col] = pd.to_numeric(
            result_df[self.start_time_col], errors="coerce"
        )
        result_df[self.end_time_col] = pd.to_numeric(
            result_df[self.end_time_col], errors="coerce"
        )
    ...
    def _load_audio_segment(
        ...
        if not math.isnan(start_time) and not math.isnan(end_time):
            ...
            if start_sample < end_sample:
                ...
                warnings.warn(
                    f"Invalid segment [{start_time}, {end_time}] for {filepath}"
                )
    ...
    def _time_stretch(self, waveform: torch.Tensor) -> torch.Tensor:
        """
        Apply time stretching via low-quality resampling.
