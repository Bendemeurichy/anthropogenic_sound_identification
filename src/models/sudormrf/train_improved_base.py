"""
Script for training the base improved model of sudormrf, adapts original training code to use custom dataset and dataloader.
Purpose is to get higher pretrained base models for the improved model, which should lead to better performance when fine-tuning on the downstream tasks.
First get proper general separation, then transfer learning to specific class of interest.
dataframe structure:
...
from base.sudo_rm_rf.dnn.losses.sisdr import (
    PairwiseNegSDR,
    PermInvariantSISDR,
    PITLossWrapper,
)
...
def validate_epoch(
    ...
    val_loss_func = PermInvariantSISDR(
        n_sources=2,
        zero_mean=True,
        backward_loss=False,
        improvement=True,
        ...
        for sources in pbar:
            ...
            with autocast_ctx:
                ...
                loss = val_loss_func(
                    outputs.float(),
                    clean_wavs.float(),
                    initial_mixtures=mixture.unsqueeze(1),
                )
...
def create_model(config: Config):
    ...
    if not hasattr(model, "n_least_samples_req"):
        model.n_least_samples_req = (config.model.enc_kernel_size // 2) * (
...
def train(config: Config, timestamp: str | None = None):
    ...
    # Setup training
    criterion = PITLossWrapper(PairwiseNegSDR("sisdr"), pit_from="pw_mtx")
        ...
        if run_validation:
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(
                    ...
                    checkpoint_dir
                    / f"best_model_pretrained_{config.model.num_blocks}blocks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt",
                )
...
def main():
    ...
    # Target classes are now provided in the configuration file.  Read the list
    # from the loaded config object instead of hard‑coding it here.
