    ...
    def compute(
        ...
        with torch.no_grad():
            for batch_x, batch_y in val_loader:
                ...
                batch_x = inputs.input_values.to(device)
...
def compute_optimal_threshold(
    model: PlaneClassifierAST, val_loader: DataLoader, device: str, metric: str = "f1"
...
def train_epoch(
    ...
    for batch_x, batch_y in pbar:
        # Convert raw waveforms to spectrograms via ASTFeatureExtractor
        inputs = feature_extractor(
            batch_x.numpy(), 
            sampling_rate=sample_rate, 
            return_tensors="pt"
...
def train_phase(
    ...
    Args:
        model: PlaneClassifierAST model
    ...
    print(f"\n{'='*70}")
    print(
        f"{phase_name.upper()}: {'TRAINING CLASSIFIER HEAD (AST frozen)' if is_phase1 else 'FINE-TUNING ENTIRE MODEL'}"
    )
    print(f"{'='*70}")
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_epoch(
            model, train_loader, criterion, optimizer, device, config.gradient_clip_norm
        )
...
def train_plane_classifier(
    ...
    config: TrainingConfig,
    device: str,
) -> Tuple[PlaneClassifierAST, Dict[str, Any], Dict[str, Any], Dict[str, float]]:
    """
    ...
    # PHASE 1: Train classifier head with frozen AST
