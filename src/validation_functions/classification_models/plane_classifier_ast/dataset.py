...
class ASTDataset(AudioClassificationDataset):
    """
    AST-specific dataset configuration.
    
    Preconfigured with:
    - Sample rate: 16kHz (AST native sample rate)
    - Duration: 10 seconds 
...
def create_dataloaders(
    ...
    """
    # Create datasets
    train_dataset = ASTDataset(train_df, config, augment=True)
    val_dataset = ASTDataset(val_df, config, augment=False)
    test_dataset = ASTDataset(test_df, config, augment=False)
...
if __name__ == "__main__":
    ...
    print("Testing ASTDataset creation...")
    dataset = ASTDataset(dummy_df, config, augment=True)
