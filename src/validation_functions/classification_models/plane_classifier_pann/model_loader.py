...
def download_pretrained_weights(
    url: str, save_path: str, force_download: bool = False
) -> str:
    """
    ...
    if weights_path is None:
        if config.pretrained_weights_path is not None:
            ...
            weights_path = download_pretrained_weights(
                config.pretrained_weights_url, cache_dir / "Cnn14_mAP=0.431.pth"
            )
...
def save_checkpoint(
    ...
    Args:
        ...
        optimizer: Optimizer (optional for inference-only checkpoints)
...
if __name__ == "__main__":
    ...
    print(
        f"CNN14 loaded. Total parameters: {sum(p.numel() for p in cnn14.parameters()):,}"
    )
