...
from src.validation_functions.metrics import ClassificationMetrics, _filter_contaminated_backgrounds
...
# Metric computation and contamination filtering are in:
#   src.validation_functions.metrics (ClassificationMetrics, _filter_contaminated_backgrounds)
...
class ValidationPipeline:
    ...
    def _separate(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        # SeparationPipeline.separate_waveform returns dict by default; use return_dict=False
        if isinstance(self.separator, SeparationPipeline):
            return self.separator.separate_waveform(waveform, return_dict=False)
